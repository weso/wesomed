...
public class SuggestionEngineLucene extends AbstractSuggestionEngine<DirectoryLucene>{
	...
	public List<Suggestion> getSuggestions(String label, String directoryName) throws SuggestionException{
		...
		try {
			...
			is = new IndexSearcher(IndexReader.open(new SimpleFSDirectory(
					new File(directory.getDirectoryPath()))));
	 ...
	 */
	private List<SuggestionWithLabel> obtainResultsWithLabel(IndexSearcher is,
			TopDocs hits, String fieldName, String[] fields) throws SuggestionException {
	 ...
	 * @return
	 */
	private SortedSet<Suggestion> normalizeResults(double[] scores, SortedSet<Suggestion> results) {
	 ...
	 */
	private SortedSet<Suggestion> rankResults(double[] scores, SortedSet<Suggestion> results,
			double firstPercentile, double secondPercentile) {
		SortedSet<Suggestion> rank = new TreeSet<Suggestion>();
	...
	@Override
	public void indexEntities(String directoryName, String query, List<IndexLucene> indexers) throws SuggestionException {
		logger.info("Begin the entity indexation " + directoryName);
		DirectoryLucene directory = null;
		try {
			...
			logger.error("Directory not found with name " + directoryName + ". Create a new one", e);
			//If not exist the directory, create one
			IndexLucene returnedIndex = (IndexLucene)indexers.get(0);
			indexers.remove(0);
			directory = new DirectoryLucene(directoryName, directoryName, returnedIndex, indexers);
		...
		try {
			...
			iw = new IndexWriter(new SimpleFSDirectory(new File(directory.getDirectoryName())), 
